...
const VerticalNavMenuGroup = ({
  ...
  // ** Toggle Open Group
  const toggleOpenGroup = (item, parent, e) => {
  ...
  const onCollapseClick = (e, item) => {
    toggleOpenGroup(item, parentItem, e)
    if (item.navLink === undefined) e.preventDefault()
  }

  // ** Checks url & updates active item
  useEffect(() => {
    if (hasActiveChild(item, currentURL, routerProps)) {
