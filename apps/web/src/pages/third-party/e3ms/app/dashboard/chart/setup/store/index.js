...
export const updateData = createAsyncThunk(
  ...
  async (payloadData) => {
    const response = await axios.post("/api/dashboardchart", payloadData.data)
    return response
  }
)
...
export const getDashboardChartData = createAsyncThunk(
  ...
  async (params) => {
    const response = await axios.get("/api/dashboardchart/view", params)
    return {
      params,
      data: response.data,
...
export const fetchSingleRecord = createAsyncThunk(
  ...
  async (params) => {
    const response = await axios.get(`/api/dashboardchart/${params}`)
    return {
      data: response.data
    }
  }
...
export const GetOverrunAmount = createAsyncThunk(
  ...
  async (params) => {
    const response = await axios.get(
      `/api/dashboardchart/GetOverrunAmount/${params.id}/${params.month}`
    )
    return {
