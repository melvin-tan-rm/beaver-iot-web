  ...
  const generateEnergyData = () => {
    ...
    for (let day = 0; day < 7; day++) {
      for (let hour = 0; hour < 24; hour++) {
        ...
        if (day <= 4) {
          if (hour >= 9 && hour <= 20) {
            baseEnergy = getRandomNumber(800, 1000) // Higher energy usage between 800 to 1000 kWh on weekdays (11am to 8pm)
          } else {
            baseEnergy = getRandomNumber(300, 600) // Lower energy usage between 300 to 600 kWh on weekdays (other hours)
          }
        } else {
          if (hour >= 9 && hour <= 20) {
            baseEnergy = getRandomNumber(500, 800) // Higher energy usage between 500 to 800 kWh on weekends (11am to 8pm)
          } else {
            baseEnergy = getRandomNumber(150, 400) // Lower energy usage between 150 to 400 kWh on weekends (other hours)
          }
        }
        ...
        const kwh = parseFloat((baseEnergy * fluctuation).toFixed(2))

        data.push([day, hour, kwh])
      }
    }
    return data
  }
  ...
  return (
    <Card style={{ height: "100%", width: "100%" }}>
      <CardHeader className="d-flex justify-content-between align-items-sm-center align-items-start flex-sm-row flex-column">
        <CardTitle tag="h4">Energy Demand (kWh)</CardTitle>
      </CardHeader>
      <CardBody>
