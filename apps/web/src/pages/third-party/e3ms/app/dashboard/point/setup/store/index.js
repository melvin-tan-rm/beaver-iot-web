...
export const updateData = createAsyncThunk(
  ...
  async (payloadData) => {
    const response = await axios.post("/api/dashboardtab", payloadData.data)
    return response
  }
)

export const addData = createAsyncThunk("api/dashboardtab/add", async (payloadData) => {
  const response = await axios.post("/api/dashboardtab", payloadData.data)
  return response
})

// For Listing
export const getDashboardTabData = createAsyncThunk("api/dashboardtab/view", async (params) => {
  const response = await axios.get("/api/dashboardtab/view", params)
  return {
    params,
    data: response.data,
...
export const fetchSingleRecord = createAsyncThunk(
  ...
  async (params) => {
    const response = await axios.get(`/api/dashboardtab/${params}`)
    return {
      data: response.data
    }
  }
