    ...
    if (id !== undefined && id !== 0) {
      ...
      async function fetchData() {
        const response = await axios.get(
          `/api/point/getcharts?id=${id}&DefaultInterval=${interval}${dateRange}` //&FromDate=2024-01-01`
        )
        let response2 = null
        if (props.data2 !== undefined && props.data2 !== null) {
          ...
          response2 = await axios.get(
            `/api/point/getcharts?id=${props.data2.Id}&DefaultInterval=${interval}${dateRange}` //&FromDate=2024-01-01`
          )
        }
        // const response = await axios.get(`/api/point/getcharts?id=1&FromDate=${formatDate(new Date())}&DefaultInterval=Daily&ChartTypeName=Column Chart (Time-based)`)
