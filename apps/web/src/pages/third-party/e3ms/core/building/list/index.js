...
const BuildingListing = () => {
  ...
  const customToolBar = () => {
    return (
      <GridToolbarContainer>
        ...
        <GridToolbarFilterButton />
        <GridToolbarExport />
        <Button tag={Link} to="/modules/building/add" color="primary" size="sm">
          <FaPlus /> Add
        </Button>
        <GridToolbarQuickFilter
          debounceMs={2000}
  ...
  useEffect(() => {
    ...
    setColumns(DefaultSetColumnsDataGrid(store, `/modules/building/edit`))
  }, [store.allData.length])

  useEffect(() => {
    dispatch(setNav(["Building", "Building", "Listing"]))
  }, [])
      ...
      <div className="list-wrapper">
        <Card>
          <div className="list-dataTable react-dataTable">
            {isWideScreen === false ? (
              DefaultSetColumnsDataGridMobile(store, `/modules/building/edit`)
            ) : (
              <DataGrid
