...
const DashboardProfileListing = () => {
  // ** Store vars
  const dispatch = useDispatch()
  const store = useSelector((state) => state.dashboardprofile)
  ...
  const customToolBar = () => {
    return (
      <GridToolbarContainer>
        ...
        <GridToolbarFilterButton />
        <GridToolbarExport />
        <Button tag={Link} to="/modules/dashboardprofile/add" color="primary" size="sm">
          <FaPlus /> Add
        </Button>
        <GridToolbarQuickFilter
          debounceMs={2000}
  ...
  useEffect(() => {
    ...
    setColumns(DefaultSetColumnsDataGrid(store, `/modules/dashboardprofile/edit`))
  }, [store.allData.length])

  useEffect(() => {
    dispatch(setNav(["DashboardProfile", "DashboardProfile", "Listing"]))
  }, [])
      ...
      <div className="list-wrapper">
        <Card>
          <div className="list-dataTable react-dataTable">
            {isWideScreen === false ? (
              DefaultSetColumnsDataGridMobile(store, `/modules/dashboardprofile/edit`)
            ) : (
              <DataGrid
...
export default DashboardProfileListing
